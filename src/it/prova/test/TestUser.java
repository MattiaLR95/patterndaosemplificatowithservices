...
public class TestUser {
	...
	private static void testCercaTuttiQuelliCheUsernameIniziaCon(UserService userService) throws Exception {
		System.out.println("\nInizio test cercaTuttiQuelliCheUsernameIniziaCon");

		System.out.println("Trovate " + userService.cercaTuttiQuelliCheUsernameIniziaCon("ppp").size() + " persone");
	}

	private static void testCercaTuttiQuelliCreatiPrimaDi(UserService userService) throws Exception {
		System.out.println("\nInizio test cercaTuttiQuelliCreatiPrimaDi");

		System.out.println("Trovate " + userService
				.cercaTuttiQuelliCreatiPrimaDi(new SimpleDateFormat("dd-MM-yyyy").parse("01-01-2023")).size()
				+ " persone");
	}
	...
	private static void TestCercaPerCognomeENomeCheInziaCon(UserService userService) throws Exception {
		...
		System.out.println("Trovate " + userService.cercaPerCognomeENomeCheInziaCon("Rossi", "ma").size() + " persone");
	}

	private static void testAccedi(UserService userService) throws Exception {
		System.out.println("\nInizio test accedi");

		userService.inserisciNuovo(new User("Mattia", "La Rocca", "laroccamattia@virgilio.it", "qwerty", new Date()));
		
		System.out.println("la persona cercata e' "+userService.accedi("laroccamattia@virgilio.it", "qwerty"));
	}
